...
void initKafkaProducer(SimStruct *S)
{
    ...
    if (confArray != NULL)
    {
        freeConfArray(confArray, nConf + nTopicConf);
    }
...
{
    int_T numInports = 1 + P_USE_EXT_KEY;
    ...
    if (!ssSetNumInputPorts(S, numInports))
        return;
    ssSetInputPortWidth(S, 0, P_MSG_LEN);
    ssSetInputPortDataType(S, 0, SS_INT8);
    ssSetInputPortRequiredContiguous(S, 0, true); /*direct input signal access*/
    ssSetInputPortDirectFeedThrough(S, 0, 1);
    if (P_USE_EXT_KEY)
    {
        ssSetInputPortWidth(S, 1, P_EXT_KEY_LEN);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    if (P_USE_EXT_KEY)
    {
        int8_T *u2 = (const int8_T *)ssGetInputPortSignal(S, 1);
        key = (char *)u2;
    }
    else
    {
