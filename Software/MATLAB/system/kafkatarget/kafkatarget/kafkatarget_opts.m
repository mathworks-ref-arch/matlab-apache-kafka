function [rtwoptions, rtwgensettings] = kafkatarget_opts()
    % kafkatarget_opts Options and settings for the kafkatarget_opts
    % Called from kafkatarget.tlc
    %
    % Copyright 2019 The MathWorks, Inc.
    
    oIdx = 1;
    
    rtwoptions(oIdx).prompt         = 'Kafka Target code generation options';
    ...
    rtwoptions(oIdx).tlcvariable    = 'KafkaDockerImageVersion';
    rtwoptions(oIdx).makevariable   = 'KAFKA_DOCKER_IMAGE_VERSION';
    
    oIdx = oIdx + 1;
    
    dockerImages = kafka.docker.getBaseImageNames();
    dockerStr = join(dockerImages, '|');
    rtwoptions(oIdx).prompt         = 'Docker base image';
    rtwoptions(oIdx).type           = 'Popup';
    rtwoptions(oIdx).popupstrings   = dockerStr{1};
    rtwoptions(oIdx).default        = dockerImages{1};
    rtwoptions(oIdx).tlcvariable    = 'KafkaDockerBaseImage';
    rtwoptions(oIdx).makevariable   = 'KAFKA_DOCKER_BASE_IMAGE';
    rtwoptions(oIdx).enable         = 'on';
    rtwoptions(oIdx).tooltip        = sprintf('Pick the docker base image for your Dockerfile');
    ...
    rtwoptions(oIdx).tlcvariable    = 'KafkaBuildDockerImage';
    rtwoptions(oIdx).makevariable   = 'KAFKA_BUILD_DOCKER_IMAGE';
    ...
    rtwoptions(oIdx).tlcvariable    = 'KafkaUseThreadsForTiming';
    rtwoptions(oIdx).makevariable   = 'KAFKA_USE_THREADS_FOR_TIMING';
    rtwoptions(oIdx).tooltip = sprintf([...
    ...
    rtwgensettings.BuildDirSuffix = '_kafka_rtw';
    rtwgensettings.Version = '1';
    rtwgensettings.DerivedFrom = 'ert.tlc';
    rtwgensettings.SelectCallback = 'kafkatarget_cb(hDlg, hSrc, ''select'')';
    rtwgensettings.ActivateCallback = 'kafkatarget_cb(hDlg, hSrc, ''activate'')';
    rtwgensettings.PostApplyCallback = 'kafkatarget_cb(hDlg, hSrc, ''postapply'')';
...
%     oIdx = oIdx + 1;
%
%     rtwoptions(oIdx).prompt         = 'Remove error status field in real-time model data structure';
%     rtwoptions(oIdx).type           = 'Checkbox';
%     rtwoptions(oIdx).default        = 'off';
