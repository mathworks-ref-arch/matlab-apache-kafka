...
function setDispString(blk)
    ...
    useExtTimestamp = strcmpi('on', get_param(blk, 'sfpExtTimestamp'));
    ...
    inputIdx = 1;
    D = sprintf('%sport_label(''input'', %d, ''Msg'');\n', D, inputIdx);
    %     D = [D, ...
    %         'port_label(''input'', 1, ''Msg'');', newline];
    if useExtKey
        inputIdx = inputIdx + 1;
        D = sprintf('%sport_label(''input'', %d, ''Key'');\n', D, inputIdx);
        %         D = [D, ...
        %             'port_label(''input'', 2, ''Key'');', newline];
    end
    if useExtTimestamp
        inputIdx = inputIdx + 1;
        D = sprintf('%sport_label(''input'', %d, ''Timestamp'');\n', D, inputIdx);
        %         D = [D, ...
        %             'port_label(''input'', 2, ''Key'');', newline];
    end
